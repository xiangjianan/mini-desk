...
4. **更新可靠**：后台发新版后，在线用户经"每日检查 + 红点 + 主动更新"拿到新版（2026-09-13 导航改 SWR 后，普通刷新不再立即换新版，下次打开或点红点即得）；不另造更新 UI。
...
| 请求 | 策略 | 理由 |
|---|---|---|
| `request.mode === "navigate"` | SWR：命中缓存零网络等待立即返回，后台拉新 `index.html` 并预取其引用的新哈希资产；未命中走网络并写缓存 | 消灭弱网打开 PWA 的白屏等待（打开即出内容）；保鲜靠每次打开的后台 revalidate，下次打开即新版 |
...
缓存的 `index.html` 由导航 SWR 策略在**每次打开时后台 revalidate**（`cache: "reload"` 绕过 HTTP 缓存）覆盖保鲜，并同步预取新 HTML 引用的哈希资源；新哈希资源另按需从网络加载并缓存。普通发版因此不产生新 SW、不进入 waiting 状态、不打扰用户。仅当 sw.js 自身逻辑/缓存结构变化时才 bump 版本号强制重建缓存。
...
→ 导航缓存未命中走网络拿新 index.html → 新哈希资源加载并重建缓存
```

不点红点的在线用户：每次打开时导航 SWR 后台拉新 `index.html` 并预取新哈希资产，**下次打开**即新版（本次仍用已渲染的旧 shell，属 SWR 的固有一步滞后）。清空缓存后 SW 不重装（sw.js 未变），缓存由导航/资源请求按需重建，自愈。
...
- 离线打开页面：导航直接命中缓存 `index.html`（零网络等待）→ 哈希资源全在缓存 → 应用完整可用；后台 revalidate 失败静默跳过，下次在线再补；版本检查 fetch 失败被现有 `catch → null` 吞掉，无感。
- 服务器宕机：同上，用户无感知（核心诉求）。
...
- `service-worker.test.ts`：vm 注入 mock 的 `self`/`caches`/`fetch` 加载 `public/sw.js`，断言：
  - 导航请求：缓存命中零网络等待立即返回（网络挂起也不阻塞，消灭白屏）；后台 revalidate 刷新缓存并预取新资产，失败/非 200 不覆盖缓存；缓存未命中走网络并写缓存；
...
- **2026-09-13**（导航策略改 SWR，消灭移动端弱网白屏）：
  1. **动机**：原「网络优先 + 3s 超时」让每次打开 PWA 都先等 `index.html` 网络响应（弱网最长 3s 白屏），缓存明明在本地却不用。
  2. **导航改 SWR**：命中缓存立即返回（零网络等待，打开即出内容），后台 `cache: "reload"` 拉新 shell 写入缓存并**预取其引用的新哈希资产**（保持「新 HTML 入缓存 → 离线也完整」不变量）；未命中（首访/清缓存后）走网络并写缓存。删除 3s 超时竞赛。
  3. **更新链路不受影响**：通道 1（每日 `no-store` 版本轮询，非导航不拦截）与通道 2（sw.js 字节比对）照常；红点点击后的清缓存刷新落在缓存未命中路径，行为同前。
  4. `CACHE_VERSION` bump 至 `mini-desk-v2`（结构性策略变化，按 §4 约定强制重建缓存一次）。
