...
CSS Grid 4-column layout (`.workbench-grid`, assembled by `src/components/WorkbenchShell.vue`): the four zones split the full page width left-to-right in the ratio (1−0.618):0.618:0.618:1 (`PHI`-derived `DEFAULT_COLUMN_WEIGHTS` = [1−1/φ, 1/φ, 1/φ, 1], normalized ≈ 14.6%/23.6%/23.6%/38.2%) — Workspace (notes + storage) is the golden emphasis. The weights describe the divider-to-divider on-screen span; the outer zones hug the page edges (grid gap/padding are 0, and `splitWidthByWeights` in WorkbenchShell.vue divides the full width by weight so the ratios hold exactly and survive window resizes). Every zone is collapsible via a rail and resizable via separator handles.
...
The mobile-inbox relay is self-hosted: `server/` is a Flask + MySQL app (legacy-Worker-compatible protocol plus pairing-code registration — `POST /inbox/<key_hash>/register` (desktop registers codes on startup and on save/rotate; `INSERT IGNORE`, revocation is permanent), `GET /inbox/<key_hash>/status` (mobile verifies on code entry, network failures fail open), POSTs to unregistered codes return 404 `unknown_code` and to revoked codes 410; the `pairing_keys` table is created by `server/migrations/2026-08-26-pairing-keys.sql` (run once as root before deploying)). `POST /polish/<key_hash>`（桌面端智能粘贴/AI润色：`kind∈todo/note/quick` + `text`≤2000 字符 + 可选 `style∈tech/concise/casual`（AI润色子菜单风格，非法值 400，缺省=默认润色口径；quick 只共享校验不使用）；todo/note 同步返回 `{items:[...]}`，quick 返回 `{button:{title,type,value}}`（title≤15 字、value 确定性回填 link=原文 URL/text=原文，LLM 只命名判型；服务端先抓链接页面 meta 作上下文——SSRF 防护 + 跨跳 10s 总预算 + 256KB/3 跳上限），LLM 失败一律 `200 {items:null,fallback:true}` / `{button:null,fallback:true}`，鉴权同注册制） Mobile captures are sent as **plaintext JSON** (`{"kind","text","polish?"}`) — E2E crypto was dropped once the relay moved to the self-hosted box. The capture page's「AI 润色」toggle (title-row far right, default off, persisted in localStorage `mini-desk-inbox-polish`, helpers in `src/sync/capturePrefs.ts`) controls server-side polishing per send: explicit `polish:false` skips the LLM and stores the raw text as-is; a missing/non-bool flag means polish (legacy-page default). The server polishes each capture with DeepSeek in a background thread before storing (`server/llm.py`, `DEEPSEEK_API_KEY` env var, 30s timeout): POST acks immediately without storing, `kind=todo` is split into one row per reminder (`#N` id suffixes), `kind=note` is summarized into numbered lines, and any LLM failure falls back to storing the raw text. Non-JSON payloads (SW-cached old capture pages still encrypting) are stored as-is without polishing; the desktop decodes plaintext-JSON rows first and falls back to AES-GCM decryption for legacy rows. The key lives only in `/opt/minidesk-inbox/.env` (sourced by `server/run.sh` on restart); if it's missing the relay logs `[llm] DEEPSEEK_API_KEY 未配置` to stderr and stores raw text without polishing. It runs on the aliyun host at `/opt/minidesk-inbox` behind nginx on `https://relay.minidesk.online:8443`; `server/deploy.sh` rsyncs and restarts it. Origin gate (`enforce_origin` in `app.py`): any request whose `Origin` header is outside the `ALLOWED_ORIGINS` whitelist gets `403 origin_not_allowed` (preflights included — cross-site JSON POSTs die at preflight); requests without `Origin` (curl/probes) and `/healthz` pass through; an unset whitelist fails closed (empty = reject every Origin). Production whitelist: `https://minidesk.online,https://todolist-7i5.pages.dev` (in `/opt/minidesk-inbox/.env`, edited manually — deploy.sh excludes it). Local backend tests: `cd server && ./.venv/bin/python -m pytest` (needs MySQL on 127.0.0.1:3306, root passwordless). Local relay testing: point `VITE_INBOX_WORKER_URL` in `.env.local` at `http://127.0.0.1:8787` and run `ALLOWED_ORIGINS=http://localhost:5173,http://127.0.0.1:5173 gunicorn -b 127.0.0.1:8787 app:app` from `server/` (without it the fail-closed gate 403s every page request).

桌面端智能粘贴：提醒事项/便签右键菜单「智能粘贴」把剪贴板全文发到 `/polish`（提醒区拆条成待办、便签区排版润色），结果直接插入、任何失败退化为原文粘贴并气泡提示；快捷动作区同款入口是每个标签头加号左侧的按钮（`desk-ai-button`）、面板头部加号左侧的按钮（产物固定落「其他」无标签分组）与空白区右键菜单「粘贴」下方的「智能粘贴」项（渐变流动走共享 `renderPolishMenuLabel`），`kind=quick` 生成一个快捷按钮（防重 pending、切工作区/卸载丢弃迟到结果、失败退化为 `classifyQuickText` 普通粘贴语义，编排走 `runQuickSmartPaste`、quick 请求超时 50s）；便签区选中文本后右键另有「AI润色」（原「智能润色」，空心星星图标）——子菜单选风格（`style∈tech技术/concise简洁/casual口语`，随请求体下发 `/polish`），对选中文字走同一润色流程并替换选区（替换后取消选区：光标塌缩并清除记忆选区，避免旧偏移错位复活），失败/超长保留原文。配对码为全局 `state.polishCode`（首次使用生成并注册，清空数据时注销）；编排器在 `src/utils/smartPaste.ts`，网络层在 `src/sync/polishClient.ts`，气泡经各面板 `polishMessage` 事件上浮到 App.vue 的 `showBubbleText`。快捷区标签头右键菜单 =「标签管理」（置顶，打开与面板菜单同款标签管理器）+「移动到空间」（有其他空间时）+「删除」（连标签下全部按钮一并删，区别于标签管理器删除=只删标签、按钮归未分组；无其他空间时菜单也打开，不回落区域菜单）。贴图区头部（计数与新增按钮之间）有「粘贴」按钮，一键追加剪贴板图片（复用右键粘贴的 `paste` 事件与读剪贴板/降级链路）。读剪贴板链路 = `navigator.clipboard.read()`，直贴位图截图；所有读不到图片的失败路径（无 API/权限拒绝/无图片类型/类型读取失败——Windows 外部复制的图片常以 DIB/文件引用落剪贴板，只在 paste 事件面上可见）都把粘贴意图挂到 `pendingBrowserImagePasteRequest` 并气泡引导 Ctrl+V：紧接着的真实 Ctrl+V 由 document 级 `handlePaste` 按菜单原落位插入（窗口 blur 即弃置挂起意图，防止陈旧落位劫持后续粘贴）。
