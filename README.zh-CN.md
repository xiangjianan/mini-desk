...
Mini Desk 是一个 AI Coding 实践项目：产品构思、交互取舍与审美判断来自人类，从界面、状态管理到自建中转服务，代码实现 100% 由 AI（Claude Code）在对话中完成。

无需注册登录，所有数据全部保存在浏览器本地：页面状态在 `localStorage`，图片原文和自定义 GIF 资源在同站点 IndexedDB。除手机速记与智能粘贴共用的自建中继外不依赖任何后端服务，适合作为个人浏览器工作台使用。
...
`npm run preview` 构建并以生产模式预览，是验证离线能力的唯一方式（开发模式不会注册 Service Worker）。局域网真机调试使用 `npm run preview:lan`（自签名 HTTPS，手机速记的 API 需要安全上下文，设备需接受一次证书提示）。
...
- 已配对后配对码中间四位打码展示，点按仍可复制完整码。
- 内容以明文 JSON 经 HTTPS 中转到自建服务器，仅以配对码哈希鉴权；可选「AI 润色」由服务端整理后入库（任何失败退存原文）。
- 条目送达桌面端后即从队列移除，30 天过期。
...
手机速记的中转只存储速记条目本身（自建服务器，仅以配对码哈希鉴权），条目送达桌面端或 30 天过期后即不可再读，读取后即从队列移除。中转不保存看板数据、图片或任何账号信息。

## 移动端说明

看板为桌面端工作流设计：视口宽度不超过 900px 时显示引导页，提示在电脑浏览器打开。已配对手机速记的工作区，手机可通过配对地址进入速记页录入提醒事项与便签，内容经自建中转同步到桌面端。

本地真机调试使用 `npm run preview:lan`；手机速记的 API 需要安全上下文，设备需接受一次自签名证书提示。
...
- `src/sync/`：手机速记的配对码、旧版载荷解码与中转客户端。
...
换句话说，这里描述的所有功能——从四区域工作台到手机速记——都是「说清楚需求 → AI 实现 → AI 自查 → AI 发布」这个循环的产物。
