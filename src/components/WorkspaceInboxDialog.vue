...
// 编辑草稿以 props 里的既有配对为初值；保存时一次性 emit，取消则原样丢弃。
...
<template>
  ...
  >
    ...
    <template v-else>
      ...
      <label class="workspace-inbox-field">
        <span class="workspace-inbox-label">{{ text.app.inboxTodoTargetLabel }}</span>
        <NSelect v-model:value="todoListId" data-testid="inbox-todo-target" :options="todoOptions" />
      </label>

      <label class="workspace-inbox-field">
        <span class="workspace-inbox-label">{{ text.app.inboxNoteTargetLabel }}</span>
        <NSelect v-model:value="noteTarget" data-testid="inbox-note-target" :options="noteOptions" />
      </label>
    </template>
