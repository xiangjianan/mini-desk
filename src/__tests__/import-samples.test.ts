...
/**
 * Guards the demo import files in samples/ against format drift: each file must
 * clear the same normalization pipeline the "导入空间" feature runs. Content is
 * validated structurally (not against wall-clock time) so the suite never rots;
 * the demo reminder dates themselves span 2026-08 → 2027-03.
 */
describe("sample import files", () => {
  const files = readdirSync(SAMPLES_DIR).filter((name) => name.endsWith(".json"));

  it("ships the three demo workspaces", () => {
    expect(files.sort()).toEqual(["人生清单.json", "前端进阶学习.json", "我的工作台.json"]);
  });

  for (const file of files) {
    it(`imports ${file} cleanly`, () => {
      const payload = JSON.parse(readFileSync(join(SAMPLES_DIR, file), "utf-8"));
